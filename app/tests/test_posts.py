...
# Test 4: Đăng ký với username đã tồn tại
def test_signup_existing_username(client):
    ...
    assert b'UsernameExists' in response.data

# Test 5: Truy cập trang bí mật khi chưa đăng nhập
def test_secret_page_unauthorized(client):
    response = client.get('/secretpage', follow_redirects=True)
    assert b'UserAuth' in response.data

# Test 6: Truy cập trang bí mật khi đã đăng nhập
def test_secret_page_authorized(client):
    client.post('/UserAuth', data={
        'username': 'testuser',
        'password': 'testpass'
    })
    response = client.get('/secretpage')
    assert response.status_code == 200
    assert b'secretpage' in response.data
...
# Test 9: Cập nhật thông tin user
...
def test_delete_user(client):
    user = User.query.first()
    response = client.delete(f'/delete_user/{user.id}')
    assert response.status_code == 200
    assert User.query.get(user.id) is None

# Test 11: Tạo tài khoản qua API
def test_create_acc_api(client):
    response = client.post('/CreateAcc', json={
        'Create_UserName': 'apiuser',
        'Create_PassWord': 'ApiPass123',
        'Create_FullName': 'API User',
        'Create_UserRole': 'user'
    })
    assert response.status_code == 200
    assert User.query.filter_by(UserName='apiuser').first() is not None

# Test 12: Đổi mật khẩu thành công
def test_change_password(client):
    client.post('/UserAuth', data={
        'username': 'testuser',
        'password': 'testpass'
    })
    response = client.post('/ChangePassWord', json={
...
# Test 13: Đổi mật khẩu không khớp
...
# Test 14: Kiểm tra trạng thái đăng nhập
def test_check_login(client):
    response = client.get('/checklogin')
    assert response.status_code == 200
    # Đăng nhập rồi kiểm tra lại
    client.post('/UserAuth', data={
        'username': 'testuser',
        'password': 'testpass'
    })
...
# Test 15: Chuyển hướng sau khi đăng nhập với next parameter
def test_login_with_next(client):
    response = client.post('/UserAuth?next=secretpage', data={
        'username': 'testuser',
        'password': 'testpass'
    }, follow_redirects=True)
    assert b'secretpage' in response.data